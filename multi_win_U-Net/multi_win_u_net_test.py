...
class Test():
        ...
        def __model(self, tf_mix):
                ...
                tf_input_spec = tf.concat([tf_mag_spec_mix, tf_mr1_mag_spec_mix, tf_mr2_mag_spec_mix], 3)
                print(tf_input_spec.shape)
                tf_input_spec = tf_input_spec[:,:,:1024,:]

            
                u_net = UNet(
                        input_shape =  tf_input_spec.shape[1:]
                )
        ...
        def __call__(self):                                              
                ...
                saver = tf.train.import_meta_graph('./../results/model/multi_win_u_net/multi_win_u_net_1998.ckpt.meta')
                with tf.Session(config = config) as sess:
                        saver.restore(sess, './../results/model/multi_win_u_net/multi_win_u_net_1998.ckpt')
...
#    scipy.io.savemat("./../results/mat/u_net_pp_sdr.mat", {'u_net_pp_sdr_list':sdr_list})
#    scipy.io.savemat("./../results/mat/u_net_pp_sir.mat", {'u_net_pp_sir_list':sir_list})
#    scipy.io.savemat("./../results/mat/u_net_pp_sar.mat", {'u_net_pp_sar_list':sar_list})
    file_path = './../results/audio/MRUNet_pp/singing_voice_separation/'
